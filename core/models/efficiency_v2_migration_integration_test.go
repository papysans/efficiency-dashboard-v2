...
func assertEfficiencyV2Columns(t *testing.T, db *sql.DB) {
	...
	expected := map[string]map[string]string{
		...
		"needs": {
			...
			"merge_ts":                            "timestamp with time zone",
			"dev_duration_min":                    "double precision",
			"cycle_duration_min":                  "double precision",
			...
			"wait_for_review_min":                 "double precision",
...
func assertEfficiencyV2Indexes(t *testing.T, db *sql.DB) {
	...
	expected := []expectedIndex{
		...
		{"conversation_events", "idx_conversation_events_task_start", false, []string{"task_id", "event_start_ts"}},
		{"conversation_events", "idx_conversation_events_source_quality", false, []string{"source", "parse_quality"}},
