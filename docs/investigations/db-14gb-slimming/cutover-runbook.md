...
> 适配「**只能更新 3 个镜像、只能跑 compose 命令、碰不到 psql**」的内网环境：所有诊断与操作都是 kbcli 子命令，经 `docker compose run --rm kbcli <cmd>` 触发。代码已全部在本地真 PG 端到端验证。
...
```bash
docker compose run --rm kbcli db-diag          # 抽样估算，快；--full 精确
```
...
```bash
docker compose run --rm kbcli slim-indexes --dry-run        # 列出将删的 10 个
docker compose run --rm kbcli slim-indexes --concurrently   # CONCURRENTLY 避锁，删
docker compose run --rm kbcli db-diag                        # H 节确认 conv_events 索引 14→4
```
...
```bash
docker compose run --rm kbcli offload-content --dry-run      # 看待卸载行数(COALESCE 含 NULL 存量行)
docker compose run --rm kbcli offload-content                # 落盘+写指针+置空列，幂等可重跑
```

## 阶段 5 — 🔴 0 退化复核（卸载前后对比，全程 db-diag）
**卸载前**先存一份 db-diag 的 **G 节（parse_quality 分布）+ G2 节（needs/user_prod_v2 行数）** 作基线，卸载/重算后再跑一次对比：
```bash
docker compose run --rm kbcli efficiency-v2     # 重算(走 A6 回读)
docker compose run --rm kbcli db-diag           # G/G2 节必须与基线一致(exact 不减/degraded 不增)
```
+ 前端抽查任一任务详情「对话历史」正文正常显示（验 server 读回 OK）。**任一退化→回滚**（多半是阶段3共享存储没配对）。

## 阶段 6 — 空间回收
```bash
docker compose run --rm kbcli db-diag           # C 节看死元组；A 节看库/表大小
```
...
```bash
docker compose run --rm kbcli clean --before 2026-05-25 --dry-run   # --before = 回看窗口下界
...
## 命令清单（都经 docker compose run --rm kbcli）
| 命令 | 作用 | 破坏性 |
|---|---|---|
