// Need reason 码翻译 —— 从 Vue frontend/src/utils/reasonText.js 原样搬运。
// 正则顺序敏感（数组顺序即优先级），保持原顺序。见 research/api-contract.md §3.2。

export type ReasonTone = 'error' | 'warning' | 'info' | 'neutral'
...
function pct(x: string | number): string {
  const n = Number(x)
...
const RULES: Rule[] = [
  { re: /^outlier:actual_to_baseline=(.+)$/, tone: 'error',
    label: (m) => `人力比异常 (${m[1]})`,
    hint: (m) => `实际人力 / 传统人力预估 = ${m[1]}，偏离正常区间（过高或过低），该需求被判为异常样本、不计入汇总。` },
  { re: /^outlier:efficiency_ratio=(.+)$/, tone: 'error',
    label: (m) => `提效比极端 (${pct(m[1])})`,
    hint: (m) => `日历提效 = ${pct(m[1])}，落在极端区间（>90% 或 <-50%），已标记为异常样本。` },
  { re: /^fusion:missing_actual_calendar$/, tone: 'warning',
    label: () => '缺少实际周期',
    hint: () => '该需求没有可测的开发时间跨度（如仅单次提交、无会话活动），dev_start 与 dev_end 相同，无法计算日历提效。' },
  { re: /^fusion:no_baselines$/, tone: 'warning',
    label: () => '无可用基线',
    hint: () => '算法 / 相似锚点 / LLM 三路工作量估算都缺失，无法给出基线，置信为未知。' },
  { re: /^fusion:single_baseline$/, tone: 'warning',
    label: () => '仅单一基线',
    hint: () => '只有一路估算可用，缺乏交叉校验，置信偏低。' },
  { re: /^band:spread_exceeds_fused$/, tone: 'warning',
    label: () => '基线离散过大',
    hint: () => '各路估算差异过大，导致提效区间下界为负、无业务意义。' },
  { re: /^need span exceeds max_need_span_days=(\d+)$/, tone: 'warning',
    label: (m) => `需求跨度超限 (${m[1]}天)`,
    hint: (m) => `开发周期超过 ${m[1]} 天上限，可能是多个需求被并入了同一边界。` },
  { re: /^covered_rule=temporal_only$/, tone: 'info',
    label: () => '按时间关联',
    hint: () => '仅按时间相邻规则关联 commit（缺少显式的需求↔提交关联证据）。' },
  { re: /^no_commits$/, tone: 'info', label: () => '无关联提交', hint: () => '该需求没有关联到任何 commit。' },
  { re: /^zero_loc_commits$/, tone: 'info', label: () => '提交无有效改动', hint: () => '关联的 commit 没有有效代码行改动。' },
  { re: /^ai_code_ratio=(.+?)<(.+)$/, tone: 'info',
    label: (m) => `AI 代码占比偏低 (${pct(m[1])})`,
    hint: (m) => `AI 代码占比 ${pct(m[1])} 低于阈值 ${pct(m[2])}，AI 贡献证据不足。` },
  { re: /^uncovered_work_ratio=(.+?)>(.+)$/, tone: 'info',
    label: (m) => `未覆盖工作偏高 (${pct(m[1])})`,
    hint: (m) => `无法归因的人工工作占比 ${pct(m[1])} 高于阈值 ${pct(m[2])}。` },
  { re: /^silica=(.+?)<(.+)$/, tone: 'info',
    label: (m) => `AI 代码匹配证据偏低 (${m[1]})`,
    hint: (m) => `Commit 级 AI 代码匹配证据 ${m[1]} 低于阈值 ${m[2]}。` },
  { re: /^knn:k=(\d+)$/, tone: 'info',
    label: (m) => `相似锚点 k=${m[1]}`,
...
export function parseReason(reason?: string | null): ReasonItem[] {
  if (!reason) return []
  return String(reason)
    ...
    .map((part): ReasonItem => {
      ...
      return { label: part, hint: part, tone: 'neutral', raw: part } // 未识别原样展示
    })
}

/** 中文短标签，用分隔符连接（列表单元格用） */
export function reasonSummary(reason?: string | null, sep = '；'): string {
...
/** 完整原因说明（tooltip 用） */
export function reasonHints(reason?: string | null): string {
  const items = parseReason(reason)
  if (!items.length) return ''
