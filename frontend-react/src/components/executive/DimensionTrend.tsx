// 通用「效率时间线」—— 从 TrendCard 抽象，按 ISO 周聚合周表行（UserProductivityV2）为
// 「每周平均提效率(%)」折线。数据策略：调用方把 /v2/efficiency 周聚合行（可选按部门成员过滤）
// 喂进来，本组件只负责按周分桶 + 渲染（小数口径 efficiency_ratio ×100 显示为百分比）。
//
// ⚠️ 仅 org/user 主体有按 user×week 的周表数据；project/repo 该端点无对应维度 → 调用方传 unavailable
// 走「数据口径诚实标注」分支，不假装有不存在的趋势。
...
import type { UserProductivityV2 } from '@/api/types'

export type TrendCaliber = 'calendar' | 'work'

interface DimensionTrendProps {
  /** 周聚合行（user×week）。调用方按主体/对象/部门成员过滤后传入。 */
  rows: UserProductivityV2[] | undefined
  loading?: boolean
  error?: string | null
  /** 提效口径：日历(efficiency_ratio) / 人力(work_efficiency_ratio)。默认日历。 */
  caliber?: TrendCaliber
  title?: string
  /** 数据口径说明（诚实标注）。 */
  subtitle?: string
  /**
   * 该主体此端点无周表数据（project/repo）→ 走「建设中/口径不适用」分支，不画图。
   * 文案由 unavailableNote 给出。
   */
  unavailable?: boolean
...
/** 周表行按 ISO 周聚合（按 week_start 落周；同周多用户行求平均提效率 %）。 */
function aggregateByWeek(rows: UserProductivityV2[], caliber: TrendCaliber): WeekPoint[] {
  const buckets = new Map<string, { sum: number; count: number; label: string; monday: number }>()
  for (const r of rows) {
    const ratio = caliber === 'work' ? r.work_efficiency_ratio : r.efficiency_ratio
...
export function DimensionTrend({
  ...
  caliber = 'calendar',
  title = '提效趋势',
  subtitle = '按 ISO 周 · 周表平均提效率',
  unavailable = false,
  unavailableNote,
}: DimensionTrendProps) {
  const { theme } = useTheme()
  const points = useMemo(() => (unavailable ? [] : aggregateByWeek(rows ?? [], caliber)), [rows, caliber, unavailable])
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          return `周一 ${item.axisValue}<br/>平均提效率：<b>${item.value.toFixed(1)}%</b><br/>样本：${pt?.count ?? 0}`
        },
      },
      xAxis: {
        type: 'category',
  ...
  return (
    <div className="glass rounded-2xl p-5 md:p-6 min-h-[20rem] hover:shadow-lg transition-shadow flex flex-col">
      <div className="flex items-center justify-between mb-4 gap-3">
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500 text-right">{subtitle}</span>
      </div>
      ...
      {unavailable ? (
        <Centered>
          <div className="flex flex-col items-center gap-2 text-center">
            <TrendIcon />
            <p className="text-sm text-gray-500 dark:text-gray-400">{unavailableNote || '该主体暂无按周的提效时间线数据'}</p>
          </div>
        </Centered>
      ) : error ? (
          ...
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            {points.length === 1 && (
              <p className="text-xs text-gray-400 dark:text-gray-500">
                本期数据集中在单周（{points[0].label} 起 · {points[0].count} 个样本 · 平均 {points[0].avgPct.toFixed(1)}%）
              </p>
            )}
          </div>
        </Centered>
